"""
FastAPI集成应用
FastAPI Integration App for xDAN-LangGraph Adapter
"""
...
# 全局适配器实例
adapter: Optional[XDANLangGraphAdapter] = None
adapter_lock = asyncio.Lock()


async def get_adapter() -> XDANLangGraphAdapter:
    """获取全局适配器实例"""
    global adapter
    async with adapter_lock:
        if adapter is None:
            print("🚀 初始化 XDANLangGraphAdapter...")
            adapter = XDANLangGraphAdapter()
            success = await adapter.initialize()
            if not success:
                raise HTTPException(status_code=500, detail="适配器初始化失败")
            print("✅ XDANLangGraphAdapter 初始化完成")
    return adapter
...
async def startup_event():
    """应用启动事件"""
    print("🚀 启动 xDAN-LangGraph Adapter API 服务...")
...
async def stream_run(assistant_id: str, thread_id: str, request: RunRequest):
